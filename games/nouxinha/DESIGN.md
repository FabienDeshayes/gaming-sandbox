...
- **Source layout:**
  ...
  | `tiles.html` | A development page, not part of the game: draws the sheet with its coordinates on it and boxes the tiles the game claims, so a tile can be found by looking at it. The way in to the other three about the sheet, which it links whatever is picked to, and to the two that are not |
  ...
  | `text.html` | A development page, not part of the game, and the other one that is not about the sheet: every string in `src/text.js`, grouped and each with a line saying when the player is looking at it, edited in place and exported as the same file back — described in §11 |
...
- **Nothing reads the game's own voice but the page that was built to.** `src/text.js` is four hundred strings declared in the order the code wants them, which is not the order a player hears them, and a string on its own says nothing about when it is on screen. `text.html` is where the copy is reviewed instead: it fetches the file as **source**, scans it for every string literal, and lists each one under the part of the game it belongs to — the HUD, the status line, the landmarks, the stones, the hall's five conversations — with a legend saying when the player is looking at it, and with an array marked out as the sequence it is (a dialog's blocks, one tap each; a list the game picks one of). The scan records each literal's **offsets**, so the page edits the file rather than a copy of its values: exporting rebuilds `src/text.js` with the edited strings swapped back in at their offsets and every other byte — comments included — untouched, which is what makes the export safe to drop straight over the file. A string nobody changed is written out verbatim, so an export with no edits in it is byte-identical to what is on disk, and an export with edits is re-scanned before it is handed over and refuses to claim it is clean if the strings no longer read back where they were. It is the same shape as the other five pages — opened through a server, drawn in a biome's colour, no palette to pick, no state in the repo and no test on it — except that the review itself is kept in `localStorage`, so a reading can be put down and picked up. It also exports the changes on their own, by path, to hand to an agent instead of saving the file.
