...
- **Blocking terrain is rock in two formations plus trees, and chests and the sorcerer are two more kinds that aren't terrain-like at all.** Rock masses and loose boulders are the same terrain drawing from the same tiles; trees are a third terrain value (`'tree'`) that blocks like rock and draws as foliage; `'chest'` and `'sorcerer'` block a step but not a light and are walked into rather than onto. Anything switching on terrain has to handle all five, and anything added to the blocked share has to keep `pickSeed` able to find an open seed.
