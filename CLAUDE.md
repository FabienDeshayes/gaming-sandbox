...
- **Light is cut back by what stands in the way.** A shape says how far a light reaches; `visibleTiles` takes an optional `isOpaque` predicate and drops everything it can't see round, and `blocksSight` in `core/world.js` is the terrain half (rock, trees, masonry always; a gate only while it is shut). Three rules are load-bearing: the blocker itself is always lit, nothing a step away can ever be hidden — which is what keeps the never-stuck promise and makes a radius-1 torch shadow-proof — and `litTiles` is the only place shape, edge choke and shadow compose.
...
- **Tests derive their routes, and live in suites.** There are no hand-authored levels, so `tests/world.js` BFSes the real world to find the nearest torch, rock or tree and every suite replays those paths. Don't hardcode coordinates. `npm test` runs `tests/all.test.js` (every suite, ~3 min); each `tests/*.test.js` also runs on its own, and `npm run test:pure` runs the seven browserless ones in about a second.
