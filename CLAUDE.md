...
- **Sprites are coordinates on one tile sheet.** `assets/tiles.png` is 49×22 tiles of 16px, 1px apart; `src/data/tiles.js` maps each sprite key to the `[col, row]` it is cut from, and that table is the only place that knows the sheet exists. The tiles are cut into 1-bit masks and baked to white textures at boot (`src/ui/textures.js`), then tinted with the palette's foreground at draw time. That tinting is what enforces the two-colour rule, so anything drawn must be tinted — a stray untinted object breaks the palette. What the sheet can't give is derived in `src/data/sprites.js`: the mirrored facing, the wizard's colour bands, the floor's dotted border. Open `games/nouxinha/tiles.html` through a server to see the sheet with its coordinates on it.
