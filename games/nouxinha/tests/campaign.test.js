...
import { PRICES, STARTING_WATER, WATER_PER_GEM } from '../src/balance.js';
import { ITEMS } from '../src/data/items.js';
import { gemColour } from '../src/config.js';
import { FIRST_GEM, GEM_ROUTE, NONCE, SANCTUMS, SEED, SHUT_GATE, bfs } from './world.js';
...
unit('walking onto a gem opens the next gate, and banking is what keeps it', () => {
...
unit('every sanctum can be walked to and back on the water its gate implies', () => {
  ...
  for (const s of SANCTUMS) {
    const route = bfs(SEED, (x, y) => x === s.centre.x && y === s.centre.y, 200, [0, 0], s.requires);
...
unit('the compass points at the nearest thing this run could actually reach', () => {
  ...
  for (let gems = 0; gems <= MAX_GEMS; gems++) {
    const run = createRun(SEED, { ...emptySave(), gems, compass: true, map: true }, NONCE);
    const target = compassTarget(run);
    const sanctum = sanctums(SEED).find((s) => s.gem === target.id);
    if (sanctum) assert(sanctum.requires <= gems, `${target.id} is offered behind a gate needing ${sanctum.requires}`);
  }

  // With everything taken and both tools owned, only the hut is left.
  const done = createRun(SEED, { ...emptySave(), gems: MAX_GEMS, compass: true, map: true }, NONCE);
  done.x = 30;
  done.y = 30;
